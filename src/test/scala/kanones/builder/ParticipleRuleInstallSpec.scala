...
class ParticipleRuleInstallSpec extends FlatSpec {


  "The ParticipleRulesInstaller object" should "throw an exception if given bad data" in {
    try {
  ...
  it should "compose FST for a valid CEX string" in {
    val goodLine = "infl.ptcp1#w_regular#wn#masc#nom#sg#pres#act#"

    val goodFst = ParticipleRulesInstaller.participleRuleToFst(goodLine)
    val expected = "<w_regular><ptcpl>wn<masc><nom><sg><pres><act><u>infl\\.ptcp1</u>"
    assert(goodFst.trim ==  expected)
  }

  it should "compose FST from CEX files in a directory" in pending   /* {
  ...
  it should "install FST in the appropriate build directory" in pending  /*
{
    val repo = File(".")
    val corpusName = "testCorpus"
