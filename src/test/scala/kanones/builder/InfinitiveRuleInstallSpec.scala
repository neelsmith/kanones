...
class InfinitiveRuleInstallSpec extends FlatSpec {


  "The InfinitiveRulesInstaller object" should "throw an exception if given bad data" in {
    try {
  ...
  it should "compose FST for a valid CEX string" in {
    val goodLine = "infl.inf1#w_regular#ein#pres#act#"

    val goodFst = InfinitiveRulesInstaller.infinitiveRuleToFst(goodLine)
    val expected = "<w_regular><infin>ein<pres><act><u>infl\\.inf1</u>"
