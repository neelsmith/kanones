...
"""
  ...
  def includeIrregAdjectives(dir: ScalaFile): Boolean = {
    val indeclSource = dir/"lexica/lexicon-irregadjectives.fst"
    indeclSource.exists && indeclSource.lines.nonEmpty
  }
  def irregAdjectiveAcceptor(dir : ScalaFile): String = {
    if (includeIrregAdjectives(dir) ) {
      """
...
$squashirregpronurn$ = <u>[#urnchar#]:<>+\.:<>[#urnchar#]:<>+</u><u>[#urnchar#]:<>+\.:<>[#urnchar#]:<>+</u> [#stemchars#]+ $gender$ $case$ $number$ <irregacc> <irregpron> <div> <irregpron> <u>[#urnchar#]:<>+\.:<>[#urnchar#]:<>+</u>
"""
    } else {""}
  }
...
"""
  ...
  def includeIrregVerbalAdjectives(dir: ScalaFile): Boolean = {
    val irregSource = dir/"lexica/lexicon-irregvadjs.fst"
    irregSource.exists && irregSource.lines.nonEmpty
  }
  def irregVerbalAdjectiveAcceptor(dir : ScalaFile): String = {
  if (includeIrregVerbalAdjectives(dir) ) {
"""
% Irregular verbal adjective acceptor
...
$squashadjurn$ = <u>[#urnchar#]:<>+\.:<>[#urnchar#]:<>+</u> <u>[#urnchar#]:<>+\.:<>[#urnchar#]:<>+</u>[#stemchars#]+<adj>$persistacc$ $=adjectiveclass$   <div> $=adjectiveclass$  <adj> [#stemchars#]* $=gender$ $case$ $number$ $degree$ <u>[#urnchar#]:<>+\.:<>[#urnchar#]:<>+</u>
""" } else { "" }
}
...
if (includeAdjectives(dir) ) {  """
$=adjectiveclass$ = [#adjectiveclass#]
$squashadvurn$ = <u>[#urnchar#]:<>+\.:<>[#urnchar#]:<>+</u> <u>[#urnchar#]:<>+\.:<>[#urnchar#]:<>+</u>[#stemchars#]+<adj>$persistacc$ $=adjectiveclass$   <div> $=adjectiveclass$  <adv> [#stemchars#]* $degree$ <u>[#urnchar#]:<>+\.:<>[#urnchar#]:<>+</u>
""" } else { "" }
}
